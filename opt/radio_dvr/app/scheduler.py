...
from opt.radio_dvr.app.recorder import RadioRecorder
...
class RadioScheduler:
    ...
    def _signal_handler(self, signum, frame):
        logger.info(f"Señal recibida: {signum}. Cerrando scheduler.")
        self.shutdown()
    ...
    def load_config(self):
        ...
        if not stations:
            raise RuntimeError("No existen emisoras configuradas.")

        for station in stations:
            station.setdefault("segmento_minutos", segmento_default)
            station.setdefault("sample_rate", sample_rate_default)
            station.setdefault("channels", channels_default)
            station.setdefault("codec", codec_default)
    ...
    def start_station(self, station):
        ...
        if recorder and recorder.is_running():
            logger.warning(f"{name} ya tiene una grabación activa.")
            return

        logger.info(f"Iniciando bloque programado para {name}")
        recorder = RadioRecorder(station)

        if recorder.start():
            self.recorders[name] = recorder
            logger.info(f"Grabación iniciada para {name}")
        else:
            logger.error(f"No fue posible iniciar la grabación de {name}")
    ...
    def stop_station(self, station):
        ...
        if recorder is None:
            logger.info(f"No existe una grabación activa para {name}")
            return

        logger.info(f"Finalizando bloque programado para {name}")
        recorder.stop()
        self.recorders.pop(name, None)
        logger.info(f"Grabación detenida para {name}")
    ...
    def health_check(self):
        for name, recorder in list(self.recorders.items()):
            if not recorder.is_running():
                logger.warning(f"La grabación de {name} terminó inesperadamente.")
                try:
                    recorder.stop()
                except Exception:
                    logger.exception(f"Error limpiando grabación de {name}")
                self.recorders.pop(name, None)
    ...
    def schedule_blocks(self, station, section_name, day_expression):
        ...
        for block in section.get("bloques", []):
            start_h, start_m, start_s = map(int, block["inicio"].split(":"))
            end_h, end_m, end_s = map(int, block["fin"].split(":"))

            start_job = f"start_{station['nombre']}_{section_name}_{block['inicio']}"
            stop_job = f"stop_{station['nombre']}_{section_name}_{block['fin']}"
    ...
    def build(self):
        ...
        for station in stations:
            ...
            self.schedule_blocks(station, "lunes_viernes", "mon-fri")
            self.schedule_blocks(station, "sabado", "sat")
            self.schedule_blocks(station, "domingo", "sun")

            after = len(self.scheduler.get_jobs())
            total_jobs += after - before

        logger.info(f"Scheduler configurado con {total_jobs} trabajos.")

    # ---------------------------------------------------------
    # Ejecución
    # ---------------------------------------------------------
    def start(self):
        logger.info("Iniciando Radio Scheduler (hora de Perú)")
        self.build()
    ...
    def shutdown(self):
        ...
        logger.info("Finalizando grabaciones activas...")

        for recorder in list(self.recorders.values()):
            try:
                recorder.stop()
            except Exception:
                logger.exception("Error deteniendo una grabación activa.")
